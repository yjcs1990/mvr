/**************************************************************************************************
 ...
 > File Name    : MvrSoundQueue.h
 > Description  : This class manages a queue of items to play as WAV files or as text to speak using a speech synthesizer.
 > Author       : Yu Jie
...
public:
   ...
   */
  MVREXPORT void addInitCallback(MvrRetFunctor<bool> *cb) { myInitCallbacks.push_back(cb);}

  /** @deprecated */
  MVREXPORT void setSpeakInitCallback(MvrRetFunctor<bool> *cb) { addInitCallback(cb);}
   ...
   * otherwise.  
   */
  MVREXPORT bool isInitialized() { return myInitialized; }
  ...
  */
  ...
  /** Add a callback functor to be invoked when playback of one sound or speech utterance starts. */
  MVREXPORT void addSoundItemStartedCallback(MvrFunctor1<MvrSoundsQueue::Item> *f)
  { myStartItemPlaybackCBList.push_back(f); }

  /** Remove a callback functor to be invoked when playback one sound or speech utterance starts. */
  MVREXPORT void remSoundItemStartedCallback(MvrFunctor1<MvrSoundsQueue::Item> *f)
  { myStartItemPlaybackCBList.remove(f);}

  /** Add a callback functor to be invoked when plackback of one sound or speech
   * utterance finishes */
  MVREXPORT void addSoundItemFinishedCallback(MvrFunctor1<MvrSoundsQueue::Item> *f)
  { myEndItemPlaybackCBList.push_back(f); }

  /** Remove a callback functor to be invoked when plackback of one sound or
   * speech utterance finishes. */
  MVREXPORT void remSoundItemFinishedCallback(MvrFunctor1<MvrSoundsQueue::Item> *f)
...
protected:
  ...
  void lock() { myQueueMutex.lock(); }
  void unlock() { myQueueMutex.unlock();}
  bool tryLock() { return myQueueMutex.tryLock(); }
