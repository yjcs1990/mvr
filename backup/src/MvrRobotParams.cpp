...
void MvrRobotParams::internalAddToConfigDefault(void)
{
  ...
  addParam(MvrConfigArg("SonarNum", &myNumSonar, "Number of sonars on the robot"), 
           section.c_str(), MvrPriority::NORMAL);
...
MVREXPORT void MvrRobotParams::addLaserToConfig(int laserNumber, MvrConfig *config, bool useDefaultBehavior, const char *section)
{
  ...
  config->addParam(MvrConfigArg("LaserFlipped", &laserData->myLaserFlipped, tempDescBuf),
                   section, MvrPriority::FACTORY, displayHintCheckbox.c_str(), restartLevel);
...
MVREXPORT bool MvrRobotParams::parseIRUnit(MvrArgumentBuilder *builder)
{
  ...
  mySonarMap[builder->getArgInt(0)][IR_TYPE]    = builder->getArgInt(1);
  mySonarMap[builder->getArgInt(0)][IR_CYCLES]  = builder->getArgInt(2);
  mySonarMap[builder->getArgInt(0)][IR_X]       = builder->getArgInt(3);
  mySonarMap[builder->getArgInt(0)][IR_Y]       = builder->getArgInt(4);
