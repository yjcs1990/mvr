...
bool MvrSocket::setLinger(int time)
{
  ...
  if (setsockopt(myFD, SOL_SOCKET, SO_LINGER, &lin, sizeof(lin)) != 0)
  {
    ...
    myError    = NetFail;
    MvrLog::logErrorFromOS(MvrLog::Normal, "MvrSocket::setLinger: setsockopt failed");
    return false;
  } 
  else
    return true;
...
bool MvrSocket::setBroadcast()
{
  ...
  if (setsockopt(myFD, SOL_SOCKET, SO_BROADCAST, NULL, 0) != 0)
  {
    ...
    myError = NetFail;
    MvrLog::logErrorFromOS(MvrLog::Normal, "MvrSocket::setBroadcast: setsockopt failed");
    return(false);
  }
  else
    return(true);
...
bool MvrSocket::getSockName()
{
  ...
  if (getsockname(myFD, (struct sockaddr *)&mySin, &size) != 0)
  {
    ...
    myError = NetFail;
    MvrLog::logErrorFromOS(MvrLog::Normal, "MvrSocket::getSockName: getSockName failed");
    return(false);
  }
...
/// Normally, write() should be used instead. This is a wrapper around the sendto() system call.
MVREXPORT int MvrSocket::sendTo(const void *msg, int len, struct sockaddr_in *sin)
{
  int ret;
...
void MvrSocket::separateHost(const char *rawHost, int rawPort, char *useHost, size_t useHostSize, int *port)
{
  if (useHost == NULL)
  {
