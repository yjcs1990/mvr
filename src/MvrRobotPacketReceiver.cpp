/**************************************************************************************************
 ...
 > File Name    : MvrRobotPacketReceiver.cpp
 > Description  : Given a device connection it receives packets from the robot through it
 > Author       : Yu Jie
 ...
 *  should be left as the default in nearly all cases, ie don't mess with it
 */
MVREXPORT MvrRobotPacketReceiver::MvrRobotPacketReceiver(bool allocatePackets, unsigned char sync1, unsigned char sync2) :
 ...
 * should be left as the default in nearly all cases, ie don't mess with it
 */
MVREXPORT MvrRobotPacketReceiver::MvrRobotPacketReceiver(MvrDeviceConnection *deviceConnection,
 ...
 */
MVREXPORT MvrRobotPacketReceiver::MvrRobotPacketReceiver(MvrDeviceConnection *deviceConnection,
          bool allocatePackets, unsigned char sync1, unsigned char sync2,
...
MVREXPORT void MvrRobotPacketReceiver::setDeviceConnection(MvrDeviceConnection *deviceConnection)
{
  myDeviceConn = deviceConnection;
}
...
MVREXPORT MvrRobotPacket *MvrRobotPacketReceiver::receivePacket(unsigned int msWait)
{
  ...
  char buf[256];
  int count = 0;
  // state can be one of the STATE_enums in the class
  ...
  if (packet == NULL || myDeviceConn == NULL || myDeviceConn->getStatus() != MvrDeviceConnection::STATUS_OPEN)
  {
    if (myTracking)
      MvrLog::log(MvrLog::Normal,
                  "%s: receivePacket: connection not open", myTrackingLogName.c_str());
  ...
  {
    MvrLog::log(MvrLog::Normal,
                "MvrRobotPacketReceiver::receivePacket() error adding msecs (%i)", msWait);
  }
  ...
  if (dynamic_cast<MvrLogFileConnection *>(myDeviceConn))
  {
    ...
    if (numRead > 0)
    {
      ...
      if ((myTracking) && (packet->getLength() < 10000))
      {
        ...
        MvrLog::log(MvrLog::Normal,
                    "Recv Packet: %s packet = %s", myTrackingLogName.c_str(), obuf);
      }
      return packet;
    }
    else
  ...
  do
  {
    ...
    if (myDeviceConn->read((char*)&c, 1, timeToRunFor) == 0)
    {
      ...
      if (state == STATE_SYNC1)
      {
        ...
        if (myTracking)
          MvrLog::log(MvrLog::Normal,
                      "%s: waiting for sync1 (got 0x%x).",
                      myTrackingLogName.c_str(), c);
        return NULL;
      }
      else
      {
...
MVREXPORT void MvrRobotPacketReceiver::setPacketReceivedCallback(MvrFunctor1<MvrRobotPacket *> *functor)
{
