...
MVREXPORT MvrRobotPacket* MvrRobotPacketReceiver::receivePacket(unsigned int msWait)
{
  ...
  {
    MvrLog::log(MvrLog::Normal,"MvrRobotPacketReceiver::receivePacket() error adding msecs (%i)",msWait);
  }
    ...
    if (numRead > 0)
    {
			...
			if ((myTracking) && (packet->getLength() < 10000)) 
      {
				...
				for (int i = 0; i < packet->getLength(); i++) 
        {
				...
				MvrLog::log (MvrLog::Normal, "Recv Packet: %s packet = %s", myTrackingLogName.c_str(), obuf);
			}  // end tracking		
      return packet;
    }
    else
  ...
  do
    {
      ...
      case STATE_ACQUIRE_DATA:
        ...
        /* put this in if you want to see bad checksum packets 
              ...
              packet->printHex();
        */
          MvrLog::log(MvrLog::Normal, "MvrRobotPacketReceiver::receivePacket: bad packet, bad checksum");
          state = STATE_SYNC1;
