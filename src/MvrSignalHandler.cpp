/**************************************************************************************************
 ...
 > File Name    : MvrSignalHandler.cpp
 > Description  : Signal handler class
 > Author       : Yu Jie
...
sigset_t MvrSignalHandler::ourHandlerSigSet;
...
void MvrSignalHandler::signalCB(int sig)
{
  ...
  MvrLog::log(MvrLog::Verbose,
              "MvrSignalHandler::runThread: Received signal '%s'",
 ...
 * a threaded program.
 */
MVREXPORT void MvrSignalHandler::createHandlerNonThread()
{
  int i;
  initSigMap();
  ...
  for (i=1; i<=SigPWR; ++i)
  {
    if (sigismember(&ourBlockSigSet, i))
      signal(i, SIG_IGN);
    if (sigismember(&ourHandlerSigSet, i))
      signal(i, &signalCB);
  }
}
...
/*
 * Block the given signals. Call this before calling createHandlerNonThreaded
 * or createHandlerThreaded.
...
/*
 * Unblock the given signals. Call this before calling createHandlerNonThreaded
 * or createHandlerThreaded.
 * @param sig the number of the signal
 */
...
*/
MVREXPORT void MvrSignalHandler::addHandlerCB(MvrFunctor1<int> *func, MvrListPos::Pos position)
{
...
MvrSignalHandler::MvrSignalHandler() : ourIgnoreQUIT(false)
{
