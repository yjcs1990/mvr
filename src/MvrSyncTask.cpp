...
/*
 * New should never be called to create a MvrSyncTask except to create the 
 * root node.
 */
                                   ...
                                   MvrTaskState::State *state, MvrSyncTask *parent)
{
  ...
  if (myParent != NULL)
  {
    ...
    setNoTimeWarningCB(parant->getNoTimeWarningCB());
  }
  else
  {
...
MVREXPORT MvrSyncTask::State MvrSyncTask::getState(void)
{
...
MVREXPORT MvrSyncTask *MvrSyncTask::find(MvrFunctor *functor)
{
  ...
  if (myFunctor == functor)
    return this;
  for (it = myMultiMap.begin(); it != myMultipMap.end(); ++it)
  {
    proc = (*it).second;
...
MVREXPORT MvrSyncTask *MvrSyncTask::findNonRecursive(MvrSyncTask *functor)
{
  MvrSyncTask *proc;
  std::multimap<int, MvrSyncTask *>::iterator it;
...
MVREXPORT void MvrSyncTask::addNewLeaf(const char *nameOfNew, int position,
                                       MvrFunctor *functor, MvrTaskState::State *state)
{
...
MVREXPORT void MvrSyncTask::run(void)
{
  ...
  switch (state)
  {
    ...
    case MvrTaskState::FAILURE:
      // The task isn't runing so just return
      myRunning = true;
      return;
...
/*
 * This sets functor which will be called to see if we should warn
...
/*
 * Prints the node... The dafaulted depth parameter controls how far over to 
 * prints the data (how many tabs)... it recurses down all its children
 */
MVREXPORT void MvrSyncTask::log(int depth)
{
  ...
  MvrLog::log(MvrLog::Terse, const_sast<char *>(str.c_str()));
  for (it = myMultiMap.rbegin(); it != myMultiMap.rend(); it++)
...
MVREXPORT MvrSyncTask *MvrSyncTask::getRunning(void)
{
  ...
  {
    MvrLog::log(MvrLog::Normal,
                "MvrSyncTask::getRunning: Tried to get running, but apparently nothing was running");
    return NULL;
  }
}
