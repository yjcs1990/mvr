...
MVREXPORT void MvrBasePacket::uByte4ToBuf(MvrTypes::UByte4 val)
{
  ...
  /*
  if it causes problems just put back the old code
...
MVREXPORT void MvrBasePacket::bufToStr(char *buf, int len)
{
  ...
  if (isNextGood(1))
  {
    ...
    for (i = 0; isNextGood(1) && i < (len - 1) && myBuf[myReadLength] != '\0'; ++myReadLength, ++i) {
...
/*
   This was added to get around having to cast data you put in, since the data shouldn't 
   really matter if its signed or unsigned.
   @param data character array to copy the data into
   @param length number of bytes to copy into data
*/
