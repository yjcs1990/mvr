...
MVREXPORT MvrArg::MvrArg(const char * name, int *pointer, const char * description, int minInt, int maxInt) 
{ 
  myType        = INT;
  myName        = name; 
...
MVREXPORT MvrArg::MvrArg(const char * name, double *pointer,
		      const char * description, double minDouble, double maxDouble) 
{ 
  myType          = DOUBLE;
  myName          = name; 
  myDescription   = description;
...
MVREXPORT MvrArg::MvrArg(const char * name, bool *pointer, const char * description) 
{ 
  myType = BOOL;
  myName = name; 
  myDescription = description;
...
MVREXPORT MvrArg::MvrArg(const char * name, MvrPose *pointer, const char * description) 
{ 
  myType = POSE;
  myName = name; 
  myDescription = description;
...
MVREXPORT MvrArg::MvrArg(const char * name, char *pointer, const char * description, size_t maxStrLen) 
{ 
  myType = STRING;
  myName = name; 
  myDescription   = description;
 ...
 */
MVREXPORT MvrArg::MvrArg(const char *name, MvrRetFunctor1<bool, MvrArgumentBuilder *> *setFunctor, 
...
MVREXPORT void MvrArg::log(void) const
{
  ...
  {
    case MvrArg::INVALID:
      MvrLog::log(MvrLog::Terse, "\tType: %10s.  This argument was not created properly.", "invalid");
    case MvrArg::INT:
      MvrLog::log(MvrLog::Terse, "\tType: %10s name: %12s value: %d", "int", getName(), getInt());
      if (strlen(getDescription()) != 0)
        MvrLog::log(MvrLog::Terse, "\t\tDescription: %s", getDescription());
      break;
    case MvrArg::DOUBLE:
      MvrLog::log(MvrLog::Terse, "\tType: %10s name: %12s value: %f", "double", getName(), getDouble());
      if (strlen(getDescription()) != 0)
        MvrLog::log(MvrLog::Terse, "\t\tDescription: %s", getDescription());
      break; 
    case MvrArg::STRING:
      MvrLog::log(MvrLog::Terse, "\tType: %10s name: %12s value: %s", "string", getName(), getString());
      if (strlen(getDescription()) != 0)
        MvrLog::log(MvrLog::Terse, "\t\tDescription: %s", getDescription());
      break;
    case MvrArg::BOOL:
      MvrLog::log(MvrLog::Terse, "\tType: %10s name: %12s value: %d", "bool", getName(), getBool());
      if (strlen(getDescription()) != 0)
        MvrLog::log(MvrLog::Terse, "\t\tDescription: %s", getDescription());
      break;
    case MvrArg::POSE:
      MvrLog::log(MvrLog::Terse, "\tType: %10s name: %12s value: (%.1f %.1f %.1f)",
          "pose", getName(), getPose().getX(), getPose().getY(), getPose().getTh());
      if (strlen(getDescription()) != 0)
        MvrLog::log(MvrLog::Terse, "\t\tDescription: %s", getDescription());
      break;
    case MvrArg::FUNCTOR:
      MvrLog::log(MvrLog::Terse, "\tType: %10s name: %12s", "functor", getName(), getPose().getX(), getPose().getY(), getPose().getTh());
      if (strlen(getDescription()) != 0)
        MvrLog::log(MvrLog::Terse, "\t\tDescription: %s", getDescription());
    ...
    case MvrArg::DESCRIPTION_HOLDER:
      MvrLog::log(MvrLog::Terse, "\tType: %20s Description: %s", "description_holder", getDescription());
    default:
      MvrLog::log(MvrLog::Terse, "\tType: %10s.  This type doesn't have a case in MvrArg::print.", "unknown");
      break;
  }

  if (myConfigPrioritySet)
    MvrLog::log(MvrLog::Terse, "\t\tPriority: %s", MvrPriority::getPriorityName(myConfigPriority));
}
