/**************************************************************************************************
 ...
 > File Name    : MvrSerialConnection.cpp
...
MVREXPORT MvrSerialConnection::~MvrSerialConnection(void)
{
  if (myPort != -1)
    close();
}
...
MVREXPORT int MvrSerialConnection::internalOpen(void)
{
  ...
  if (myStatus == STATUS_OPEN)
  {
    MvrLog::log(MvrLog::Terse,
                "MvrSerialConnection::internalOpen: Serial port already open");
    return OPEN_ALREADY_OPEN;
  }

  if (myIs422)
    MvrLog::log(MvrLog::Terse,
                "MvrSerialConnection::internalOpen: Connecting to serial422 port '%s'",
                myPortName.c_str());

  else
    MvrLog::log(MvrLog::Terse,
                "MvrSerialConnection::internalOpen: Connecting to serial port '%s'",
                myPortName.c_str());      
  ...
  /// turn off echo, canoical mode, extened processing, signals
  ...
  {
    MvrLog::logErrorFromOS(MvrLog::Terse,
                           "MvrSerialConnection::open: Could not set up tcflush failed");
    close();
    myStatus = STATUS_OPEN_FAILED;
    return OPEN_COULD_NOT_SET_UP_PORT;                
  }
  ...
  if (!myIs422)
  {
    if (myBaud != 0 && rateToBaud(myBaudRate) == -1)
    {
      MvrLog::logErrorFromOS(MvrLog::Terse,
                             "MvrSerialConnection::open: Invalid baud rate.");
      close();
      myStatus = STATUS_OPEN_FAILED;
  ...
  else
  {
    if (myBaudRate != 0 && rateToBaud(myBaudRate) == -1)
	  ...
	  {
		  MvrLog::log(MvrLog::Terse, "MvrSerialConnection::open: Could not set baud rate.");
		  close();
		  myStatus = STATUS_OPEN_FAILED;
		  return OPEN_COULD_NOT_SET_BAUD;
	  }
...
MVREXPORT void MvrSerialConnection::openSimple(const char *port)
{
  if (port == NULL)
...
MVREXPORT bool MvrSerialConnection::close(void)
{
  ...
  if (ret == 0)
    ...
    MvrLog::logErrorFromOS(MvrLog::Verbose,
                           "MvrSerialConnection::close: Unsuccessfully close serial port.")                ;
  myPort = -1;
  if (ret == 0)
    return true;
  else
...
MVREXPORT bool MvrSerialConnection::setBaud(int rate)
{
  ...
  {
    MvrLog::logErrorFromOS(MvrLog::Terse,
                           "MvrSerialConnection::SetBaud: Could not set output baud rate on termios struct.");
    return false;                           
  }
  if (cfsetospeed(&tio, baud)) 
  {
  ...
  {
    MvrLog::logErrorFromOS(MvrLog::Terse, 
                           "MvrSerialConnection::setBaud: Could not set input baud rate on termios struct.");
    return false;
  }
  ...
  {
    MvrLog::logErrorFromOS(MvrLog::Terse,
                           "MvrSerialConnection::setBaud: Could not set baud rate.");
    return false;
  }
...
int MvrSerialConnection::rateToBaud(int rate)
{
  switch (rate)
  {
    ...
    default:
      MvrLog::log(MvrLog::Terse,
                  "MvrSerialConnection::rateToBaud: Did not know baud for rate %d", rate);
      return -1;
  }
}
...
int MvrSerialConnection::baudToRate(int baud)
{
  switch (baud)
  {
    ...
    default:
      MvrLog::log(MvrLog::Terse,
                  "MvrSerialConnection::baudToRate: Did not know baud for baud.");
      return -1;  
  }
}
/*
...
#endif       
      MvrLog::logErrorFromOS(MvrLog::Terse,
                             "MvrSerialConnection::write: Error on writing.")
    }
    return n;
  }
  MvrLog::log(MvrLog::Terse, 
              "MvrSerialConnection::write: Connection invalid.");
  return -1;              
}

MVREXPORT int MvrSerialConnection::read(const char *data, unsigned int size, unsigned int msWait)
{
  struct timeval tp;
  fd_set fdset;
  int n;
  ...
  if (myPort >= 0)
  {
    if (msWait >= 0)
    {
      ...
      if (!timeDone.addMSec(msWait))
      {
        MvrLog::log(MvrLog::Normal, 
                    "MvrSerialConnection::read: error adding msec (%i).", msWait);
      }
      while((timeLeft = timeDone.mSecTo() >= 0)
      {
        tp.tv_sec  = (timeLeft) / 1000;
        tp.tv_usec = (timeLeft % 1000) * 1000;
        FD_ZERO(&fdset);
        FD_SET(myPort, &fdset);
        if (select(myPort+1, &fdset, NULL, NULL, &tp) <= 0)
          return bytesRead;
        if ((n = ::read(myPort, const_cast<char *>(data)+bytesRead, size-bytesRead)) == -1)
        {
          MvrLog::logErrorFromOS(MvrLog::Terse,
                                 "MvrSerialConnection::read: Blocking read failed");
          return bytesRead;                                 
        }
        bytesRead += n;
        if (bytesRead >= size)
    ...
    else
    {
      ...
      if (n == -1)
        MvrLog::logErrorFromOS(MvrLog::Terse,
                               "MvrSerialConnection::read: Non-Blocking read failed.");
      return n;                               
    }
  }
...
MVREXPORT bool MvrSerialConnection::getCTS(void)
{
  ...
  else
  {
    MvrLog::logErrorFromOS(MvrLog::Normal,
                           "MvrSerialConnection::getCTS: ioctl(TIOCMGET)");
    return false;                           
  }
}
...
MVREXPORT bool MvrSerialConnection::getDSR(void)
{
  ...
  {
    MvrLog::logErrorFromOS(MvrLog::Normal,
                          "MvrSerialConnection::getDSR: ioctl(TIOCMGET)");
    return false;
  }    
}
...
MVREXPORT bool MvrSerialConnection::getDCD(void)
{
  ...
  {
    MvrLog::logErrorFromOS(MvrLog::Normal,
                          "MvrSerialConnection::getDCD: ioctl(TIOCMGET)");
    return false;    
  }
}
...
MVREXPORT bool MvrSerialConnection::getRing(void)
{
  ...
  {
    MvrLog::logErrorFromOS(MvrLog::Normal,
                          "MvrSerialConnection::getRing: ioctl(TIOCMGET)");
    return false;  
  }
}