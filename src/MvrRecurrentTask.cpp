...
// This class must be subclassed with the particular Task that will be run
 ...
 * Here we check if a Go request has been made, and if so, we run Task()
 * When done, set running to false, and wait for the next request
 */
MVREXPORT void *MvrRecurrentTask::runThread(void *ptr)
{
  threadStarted();
#ifndef WIN32
...
MVREXPORT int MvrRecurrentTask::done()
{
  ...
  if (is_running)
    return 0;
  if (is_killed)
    return 2;       // we didn't complete, were killed
  else
    return 1;
}
