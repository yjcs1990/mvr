/**************************************************************************************************
 ...
 > File Name    : MvrRobotConnector.cpp
 ...
 * the robot.
 */
MVREXPORT MvrRobotConnector::MvrRobotConnector(MvrArgumentParser *parser, MvrRobot *robot, bool autoParseArgs, bool connectAllComponents) :
...
MVREXPORT bool ArgRobotConnector::parseArgs(MvrArgumentParser *parser)
{
  ...
  if (parser->checkArgument("-remoteIsSim") || parser->checkArgument("-ris"))
    myRemoteIsSim = true;
  if (parser->checkArgument("-remoteIsNotSim") || parser->checkArgument("-rins"))      
    myRemoteIsNotSim = true;

  if (parser->checkArgument("-robotLogPacketsReceived") || parser->checkArgument("-rlpr")) 
    myRobotLogPacketsReceived = true;
  
  if (parser->checkArgument("-robotLogPacketsSent") || parser->checkArgument("-rlps"))      
    myRobotLogPacketsSent = true;

  if (parser->checkArgument("-robotLogMovementReceived") || parser->checkArgument("-rlmr"))      
    myRobotLogMovementReceived = true;

  if (parser->checkArgument("-robotLogMovementSent") || parser->checkArgument("-rlms"))
    myRobotLogMovementSent = true;
  
  if (parser->checkArgument("-robotLogVelocitiesReceived") || parser->checkArgument("-rlvr"))
    myRobotLogVelocitiesReceived = true;
  
  if (parser->checkArgument("-robotLogActions") || parser->checkArgument("-rla"))
    myRobotLogActions = true;

  if (!parser->checkParameterArgumentString("-remoteHost", &myRemoteHost) || 
      !parser->checkParameterArgumentString("-rh", &myRemoteHost) ||
      !parser->checkParameterArgumentString("-robotPort", &myRobotPort) ||
      !parser->checkParameterArgumentString("-rp", &myRobotPort) ||
      !parser->checkParameterArgumentInteger("-remoteRobotTcpPort", &myRemoteRobotTcpPort) ||
      !parser->checkParameterArgumentInteger("-rrtp", &myRemoteRobotTcpPort) ||
      !parser->checkParameterArgumentInteger("-robotBaud", &myRobotBaud) || 
      !parser->checkParameterArgumentInteger("-rb", &myRobotBaud))      
  {
...
MVREXPORT void ArgRobotConnector::logOptions(void) const
{
